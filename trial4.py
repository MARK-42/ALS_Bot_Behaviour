...
class BotDetails:
    def __init__(self,idinput,x,y,gx,gy,vx,vy):
        self.bid=idinput
        self.bx=x
        self.by=y
        self.bgx=gx
...
class GFG:
    def __init__(self,master):
        ...
        for i in range(1):
            ...
            idinput=self.canvas.create_text(x,y,fill="red",text="X",font=('Helvetica','30','bold'))
            self.goal_list.append(ObjDetails(idinput, x, y))
        ...
        for i in range(10):
            ...
            while tmplist in self.all_coordinates:
                ...
                tmplist=[x,y]
            self.all_coordinates.append(tmplist)
            idinput=self.canvas.create_oval(x-10,y-10,x+10,y+10,fill="green")
            index = int(random.randrange(0,len(self.goal_list)))
            gx=self.goal_list[index].ox
            gy=self.goal_list[index].oy
            # if(i<5):
            #     gx=100
            #     gy=800
            # else:
            ...
            vx=1
            vy=0
            bo=BotDetails(idinput, x, y, gx, gy, vx, vy)
            self.bot_list.append(bo)
    ...
    def movers(self):
        ...
        # l=random.randrange(0,len(bot_list))
        # px=self.bot_list[0].bvx
        # py=self.bot_list[0].bvy
        c=0
        ...
        # for j in self.bot_list:
        #     d_from_rep=pow(pow(self.curr_x-j.bx,2)+pow(self.curr_y-j.by,2),0.5)
        #     if(d_from_rep<=D_REP):
        #         dist_mul=(1/d_from_rep)-(1/D_REP)
        #         F_Rep_x+=(KREP*dist_mul*(self.curr_x-j.bx))/pow(d_from_rep,2)
        #         F_Rep_y+=(KREP*dist_mul*(self.curr_y-j.by))/pow(d_from_rep,2)
        #     else:
        #         F_Rep_x+=0
        #         F_Rep_y+=0
        ...
        # for j in self.bot_list:
        #     d_from_rep2=pow(pow(self.currg_x-j.bx,2)+pow(self.currg_y-j.by,2),0.5)
        #     if(d_from_rep2<=D_REP):
        #         dist_mul=(1/d_from_rep2)-(1/D_REP)
        #         F_Rep_x2+=(KREP*dist_mul*(self.currg_x-j.bx))/pow(d_from_rep2,2)
        #         F_Rep_y2+=(KREP*dist_mul*(self.currg_y-j.by))/pow(d_from_rep2,2)
        #     else:
        #         F_Rep_x2+=0
        #         F_Rep_y2+=0
        ...
        for i in self.bot_list:
            ...
            F_Rep_x3=0
            F_Rep_y3=0
            #repulsion from first static obstacle 
            # d_from_rep3=pow(pow(i.bx-self.obstacle_x,2)+pow(i.by-self.obstacle_y,2),0.5)
            # if(d_from_rep3<=D_REP):
            #     dist_mul=(1/d_from_rep3)-(1/D_REP)
            #     F_Rep_x3=(KREP*dist_mul*(i.bx-self.obstacle_x))/pow(d_from_rep3,2)
            #     F_Rep_y3=(KREP*dist_mul*(i.by-self.obstacle_y))/pow(d_from_rep3,2)
            # else:
            #     F_Rep_x3=0
            #     F_Rep_y3=0
            ...
            # # repulsion from first goal bot 
            # d_from_rep3=pow(pow(i.bx-self.currg_x,2)+pow(i.by-self.currg_y,2),0.5)
            # if(d_from_rep3<=D_REP):
            #     dist_mul=(1/d_from_rep3)-(1/D_REP)
            #     F_Rep_x3+=(KREP*dist_mul*(i.bx-self.currg_x))/pow(d_from_rep3,2)
            #     F_Rep_y3+=(KREP*dist_mul*(i.by-self.currg_y))/pow(d_from_rep3,2)
            # else:
            #     F_Rep_x3+=0
            #     F_Rep_y3+=0
            
            # # repulsion from first q bot 
            # d_from_rep3=pow(pow(i.bx-self.curr_x,2)+pow(i.by-self.curr_y,2),0.5)
            # if(d_from_rep3<=D_REP):
            #     dist_mul=(1/d_from_rep3)-(1/D_REP)
            #     F_Rep_x3+=(KREP*dist_mul*(i.bx-self.curr_x))/pow(d_from_rep3,2)
            #     F_Rep_y3+=(KREP*dist_mul*(i.by-self.curr_y))/pow(d_from_rep3,2)
            # else:
            #     F_Rep_x3+=0
            #     F_Rep_y3+=0
                
            # calculating repulsion from all other moving bots
            for k in self.bot_list:
                if(k==i):
                    continue
            ...
            # frand=10
            # if(F_Mag3<5 and d_from_goal3>15):
            #     F_Res_x3+=frand
            #     F_Res_y3+=frand
            #     frand*=2
    ...
    def movement(self):
        ...
        self.canvas.after(20, self.movers)
